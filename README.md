...
- Linux: links against `resolv` for DNS and requires [OpenSSL](https://www.openssl.org/) (found via `find_package(OpenSSL REQUIRED)`, e.g. `apt install libssl-dev`) for hashing. It also links against `backtrace` (libbacktrace, typically bundled with GCC) for symbolized stack traces *if* a `try_compile` check at configure time (`NSTL_HAVE_BACKTRACE`, see `cmake/backtrace_is_around.cpp`) finds it; otherwise `backtrace.hpp` falls back to a no-op stub, same as on macOS
- macOS: links against `resolv` and requires OpenSSL for hashing; stack traces (`backtrace.hpp`) are a no-op stub on this platform
- Whether `nstl::c_timezone`/`nstl::log::Logger` are built on `std::chrono` directly or on the [HowardHinnant/date](https://github.com/HowardHinnant/date) library is decided at configure time by a `try_compile` check (`NSTL_USING_STD_CHRONO`, see `cmake/chrono_has_tzone.cpp`) that probes whether the standard library's `std::chrono::time_zone` support is complete — currently false on macOS/libc++, so the HH date library is built there from the `modules/date` git submodule
- Git — read at configure time (`cmake/FindNstl.cmake`) via `git rev-parse --short HEAD` to stamp the build with a version string (`nstl::version()`, see Components); falls back to a `0` build component if Git or a `.git` directory isn't found
...
| [`nstl/vector.hpp`](nstl/nstl/vector.hpp) | `nstl::vector<T>` — a header-only, `malloc`/`realloc`-backed vector restricted to trivially-copyable `T`, with `push_back()`/`pop_back()`/`reserve()`/`clear()`/`at()`/`erase()` (single element or iterator range), bounds-checked iterator/`data()` access, and `release()` to hand off ownership as a `(pointer, size, capacity)` tuple. |
| [`nstl/secure_string.hpp`](nstl/nstl/secure_string.hpp) | `nstl::strlen` / `strcmp` / `strncmp` / `wcslen` — null-safe wrappers over the C string functions (returning 0, or an arbitrary but consistent ordering, instead of crashing on a null pointer). `secure_zero()` zeroes a buffer via `memset`, for scrubbing sensitive data before it's freed. |
...
| [`nstl/version.hpp`](nstl/nstl/version.hpp) | `nstl::version()` — returns the library's `major.minor.build` version string, where `build` is the short Git commit hash of the checkout it was built from (or `0` if Git wasn't available at configure time; see Requirements). Generated at configure time from `version.cpp.in` and `cmake/FindNstl.cmake`. |
