...
| [`nstl/memory.hpp`](nstl/nstl/memory.hpp) | `nstl::observer_ptr` (alias `leaking_ptr`) — a non-owning smart pointer, header-only: constructible from a raw pointer, `unique_ptr`, or `shared_ptr`, with the usual comparison operators, `make_observer()` factories, and a `std::hash` specialization. |
