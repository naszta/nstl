...
struct process
{
    ...
    process(process&& other_) noexcept { this->swap(other_); }
    ...
    bool validPid() const { return 0 < pid; }

    bool valid() const { return this->validPid() || 0 <= read_fd; }
    ...
    void close_read(int epfd)
    {
        ...
        NSTL_THROW_EXCEPTION_IF(::epoll_ctl(epfd, EPOLL_CTL_DEL, read_fd, &ev_pipe), std::runtime_error,
                                "epoll_ctl failed");
        ::close(read_fd);
        read_fd = -1;
    }
...
std::optional<int> start_processes(int argc_, char** argv_, char** envv_, std::vector<process>& tgt_)
{
    ...
    for (std::uint32_t idx = 0; idx < tgt_.size(); ++idx)
    {
        ...
        if (pid == 0)
        {
            NSTL_THROW_EXCEPTION_IF(dup2(p_item.write_fd, STDOUT_FILENO) < 0, std::runtime_error,
    ...
    for (auto size = ::read(sfd, &ssi, ssi_size); 0 < size; size = ::read(sfd, &ssi, ssi_size))
    {
        ...
        case SIGCHLD:
            ...
            while (true)
            {
                ...
                {
                    const auto pid_itr =
                        std::ranges::find_if(processes, [pid](const process& proc) { return proc.pid == pid; });
...
int main(int argc_, char** argv_, char** envv_)
{
    ...
    NSTL_THROW_EXCEPTION_IF(::epoll_ctl(epfd, EPOLL_CTL_ADD, sfd, &ev_signal) < 0, std::runtime_error,
                            "epoll_ctl failed");
    ...
    std::ranges::for_each(processes,
                          [epfd](process& p_item)
                          {
                              ...
                              NSTL_THROW_EXCEPTION_IF(::epoll_ctl(epfd, EPOLL_CTL_ADD, p_item.read_fd, &ev_pipe),
    ...
    while (std::ranges::any_of(processes, [](const process& pitem) { return pitem.validPid(); }))
    {
        ...
        for (int idx = 0; idx < n; ++idx)
        {
            ...
            else if (const auto pitr = std::ranges::find_if(processes, [tgt_fd](const process& p_item)
                                                            { return p_item.read_fd == tgt_fd; });
                     pitr != processes.end())
            {
    ...
    std::ranges::for_each(processes,
                          [&buffer, &epfd](process& p_item) { read_pipe(p_item, std::span{ buffer }, epfd); });
