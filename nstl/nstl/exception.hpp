...
#define NSTL2_THROW_EXCEPTION_IMPL(error, detail)                              \
    do                                                                         \
    {                                                                          \
        constexpr auto _filename__ = ::nstl::safe_basename_view(__FILE__);     \
        std::ostringstream _oss__;                                             \
        _oss__ << _filename__ << ':' << __LINE__ << " - " << detail;           \
        throw ::nstl::exception{ _oss__.str(), _filename__.data(), __LINE__ }; \
    } while (false)
...
#define NSTL2_NESTED_THROW_EXCEPTION_IMPL(error, detail)                                         \
    ...
    {                                                                                            \
        ...
        std::ostringstream _oss__;                                                               \
        _oss__ << _filename__ << ':' << __LINE__ << " - " << detail;                             \
        std::throw_with_nested(::nstl::exception{ _oss__.str(), _filename__.data(), __LINE__ }); \
    } while (false)
