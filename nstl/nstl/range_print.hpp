...
public:
    Streamer(IterType&& beg_, IterType&& end_, DelimType&& delim_)
        : _beg{ std::forward<IterType>(beg_) }, _end{ std::forward<IterType>(end_) },
...
public:
    MapStreamer(IterType&& beg_, IterType&& end_, DelimType&& delim_, EqDelimType&& eq_delim_)
        : _beg{ std::forward<IterType>(beg_) }, _end{ std::forward<IterType>(end_) },
          _delim{ std::forward<DelimType>(delim_) }, _eq_delim{ std::forward<EqDelimType>(eq_delim_) }
    {
    }

    template <class CharT, class TraitsT = std::char_traits<CharT>>
    friend std::basic_ostream<CharT, TraitsT>& operator<<(std::basic_ostream<CharT, TraitsT>& os_,
                                                          const MapStreamer& str_)
    {
        if (str_._beg == str_._end)
        {
            return os_;
...
auto range_map_print_iter(IterType beg_, IterType end_, DelimType delim_, EqDelimType eq_delim_)
{
    return ::nstl::detail::MapStreamer{ std::forward<IterType>(beg_), std::forward<IterType>(end_),
                                        std::forward<DelimType>(delim_), std::forward<EqDelimType>(eq_delim_) };
}
...
auto range_map_print(RangeType&& range_, DelimType delim_, EqDelimType eq_delim_)
{
    static_assert(std::is_lvalue_reference_v<RangeType>,
                  ...
                  "returned object with dangling iterators once this expression ends. Bind the "
                  "range to a named variable first.");
    return ::nstl::range_map_print_iter(range_.begin(), range_.end(), std::forward<DelimType>(delim_),
                                        std::forward<EqDelimType>(eq_delim_));
}
} // namespace nstl
