...
public:
    ...
    explicit basic_ro_buffer(const view_type strview) : basic_ro_buffer{ strview.data(), strview.size() } {}

#ifdef __cpp_lib_span
    template <std::size_t extent = std::dynamic_extent>
    explicit basic_ro_buffer(const std::span<CharT, extent>& value) : basic_ro_buffer{ value.data(), value.size() }
    {
    }
#endif

    size_t readBytes() const { return static_cast<size_t>(this->gptr() - this->eback()); }

    view_type content() const { return view_type{ this->eback(), static_cast<size_t>(this->egptr() - this->eback()) }; }

    void swap(basic_ro_buffer& right_) noexcept { base_type::swap(right_); }
...
protected:
    ...
    int_type overflow(int_type) final { return traits_type::eof(); }
    // read only: no flush
