...
TEST(Logging, Test)
{
	...
	test_thr.join();
	logger.reset();
	EXPECT_TRUE(target.view().ends_with(expected)) << "Expected value: \"" << expected << "\"; actual: \"" << target.view() << "\"";
}
...
TEST(Logging, Multi)
{
	...
	nstl::log::logger() = [wptr = std::move(wptr)](const nstl::log::LogLevel::LogEnum level_, const std::string_view line_) {
